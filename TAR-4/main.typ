...
Для моделирования работы САР со всеми рассматриваемыми типами регуляторов достаточно построить модель системы с ПИД-регулятором как наиболее общюю. Тогда модели более простых регуляторов можно построить "отсеканием" ветвей с соответствующими сигналами $eta_1$, $eta_2$ или $sigma$. Такая модель САР с ПИД-регулятором в структурной форме изображена на @SAR-PID[рисунке].
...
Для проведения анализа этой модели средствами DifferentialEquations.jl, она должна быть представленна в стандартной форме в виде системы линейных дифференциальных уравнений. Для П- и ПД-регуляторов приведение к дифференциальной форме было произведено в работе 3, его результат показан на листингах @list_p[] и @list_pd[].
...
Функция, соответсвующая этому уравнению, показана на @list_pi[листинге].
...
Решение этого уравнения будет производиться методом Лагранжа:
...
Функция, соответсвующая уравнениям 2.3, показана на @list_pid[листинге].
...
Приведение И-регулятора к дифференциальной форме тривиально, функция, соответсвующая уравнению И-регулятора, показана на @list_i[листинге].
...
$T_4$ была найдена методом варьирования по условию минимизации велиичны перерегулирования в модели САР с ПИ-регулятором (что аналогично минимизации максимальной ошибки $phi_max$, так как $phi_infinity$ инвариантно изменению параметров ПИ-регулятора, $phi_infinity=0$) при неотрицательности ошибки $(phi_t - phi_infinity)>0$. Второе условие призвано выбрать параметры, обеспечивающие наименьшую колебательность, так как при уменьшении величины перерегулирования колебательность начинает расти.

Для демонстрации структурной неустойчивости модели САР с И-регулятором были построены переходные функции такой системы при различных значениях $T_4$.
...
+ Пропорционально-дифференциальный закон сохраняет статическую ошибку регулирования неизменной относительно пропорционального закона, однако не имеет колебательности, обеспечивает отсутсвие перерегулирования и меньшее время переходного процесса.
...
Видно, что ни при малых, ни при больших значениях $T_4$ не происходит стаблилизации выходного сигнала, однако происходит зануление статической ошибки регулирования. Таким образом, САР с интегральным регулятором структурно неустойчива.
...
Был проведён анализ влияния пропорционального, пропорционально-дифференциального, пропорционально-интегрального, пропорционально-интегрально-дифференциального и интегрального законов регуляирования на переходные процессы в САР паровой турбины.

Определено, что использование И-закона ведёт к струтурной неустойчивости, П- и ПД-законы приводят к установлению статической ошибки регулирования, тогда как ПИ- и ПИД-законы представляются оптимальными для использования в САР паровой турбины, так как обеспечивают нулевую статическую ошибку, устойчивость, а также в случае ПИД-закона отсутствие колебательности.
