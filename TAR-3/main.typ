...
Объектом исследования является система регулирования угловой скорости ротора паровой турбины, схема которой изображена на @SAR[рисунке]. В рамках этой работы считается, что паровой ёмкостью можно принебречь.
...
Методика, описанная в учебном пособии @egorshin2021, подразумевает построение логарифмических частотных характеристик размокнутой системы, по которым определяется частота среза располагаемой системы $omega_"ср"^"расп"$ и выбирается параметр $T_1 = 1 "/" omega_"ср"^"расп"$, после чего выбирается $T_2 approx 0.1 dot T_1$, из соображений максимальной устойчивости по фазе $60 degree < gamma < 90 degree$.

Однако, построние именно таких характеристик для этой цели не является самым наглядным вариантом, и явно было выбрано в качестве рекоммендуемой методики с учётом простоты их построения без специализированного ПО. Простроение АФЧХ позволяет наглядно показать запас устойчивости по фазе с помощью полярных координат, величину и положение частоты среза с помощью логарифмического цветового градиента потому что точек близко к (0,0) много, тогда как нам важно рассматривать градиент значений $omega$ у заначения $omega_"ср"$, располагающегося на единичной окружности.
...
#h(-1.25cm) откуда найдена частота среза $omega^"расп"_"ср"_1 = #w-ras-1 "рад"/c $. Определяются параметры корректирующего звена: $T_1_1 = 1 "/" omega^"расп"_"ср"_1 = #T-1-1 "с;"$ $T_2_1 = 0.1 dot T_1_1 = #T-2-1 "c."$ По этим праметрам построена изображенная на @pol_cor_3[рисунке] АФЧХ корректируемой системы с помощью передаточной функции
...
На @plot_3[рисунке] изображены графики переходных процессов располагаемой и скорректированной системы. Эффект перерегулирования ликвидирован, установившая ошибка регулирования не изменилась.
...
#h(-1.25cm) откуда найдена частота среза $omega^"расп"_"ср"_2 = #w-ras-2 "рад"/c $. Определяются параметры корректирующего звена: $T_1_2 = 1 "/" omega^"расп"_"ср"_2 = #T-1-2 "с;"$ $T_2_2 = 0.1 dot T_1_2 = #T-2-2 "c."$ По этим праметрам построена изображенная на @pol_cor_17[рисунке] АФЧХ корректируемой системы с помощью передаточной функции

$ W_"корр"_2(lambda) = 1 / (delta_omega_2 T_a^1 lambda dot (T_s_2 lambda +1)) dot (T_1_2 lambda +1)/(T_2_2 lambda + 1); $
...
На @plot_17[рисунке] изображены графики переходных процессов располагаемой и скорректированной системы. Эффект перерегулирования ликвидирован, установившая ошибка регулирования не изменилась.
