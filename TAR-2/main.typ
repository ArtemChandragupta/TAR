...
Значения параметров САР указаны в @ar[таблице].
...
Выделим из уравнения колебательного звена 2.2 характеристический многочлен и найдем его корни:

$ T^2 lambda^2 + 2 zeta T lambda + 1 => lambda_(1,2) = (-2 zeta T plus.minus sqrt(4 zeta^2 T^2 - 4T^2))/(2T^2) => \ => lambda = (-zeta + i sqrt(1 - zeta^2))/T = -a + i omega => a = zeta/T; " " omega = 1/T sqrt(1 - zeta^2), $
...
#h(-1.25cm) где #context box(baseline: 100% - measure([a]).height, grid(
  ...
  row-gutter: 0.75em,
  align: (right, left),
  $a  -$, [декремент затухания;],
  $omega -$, [частота колебаний.]
))

Так как мы рассматриваем затухающую экспаненту,

$ A_2 / A_1 = e^(-a t) => ln(A_2 / A_1) = -a dot t => a = 1/t ln(A_1/A_2), $

Возьмём $t$ как полупериод колебаний, тогда $A_1$ и $A_2$ --- амплитуды, отстоящие друг от друга на полупериод, $t = frac(pi,omega, style: "horizontal")$. Тогда
...
Откуда параметры передаточной функции колебательного звена:
...
Проведение симуляции реакции САР позволяет получить график переходной функции, показанный на @Per[рисунке]. Из него можно получить значения полупериода $t_"пп"$ и амплитуд $A_1$ и $A_2$ относительно асимптоты (установившейся ошибки регулирования). Полученные значения приведены в @arr[таблице].
...
$ a = 1 / t_"пп" ln(A_1/A_2) = 1 / #calc.round((Tmin - Tmax),digits:3) dot ln(#calc.round(A1,digits: 3) / #calc.round(A2,digits: 3)) = #calc.round(a,digits: 3) "1/с;" $
