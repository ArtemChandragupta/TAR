...
Отчет объемом #context counter(page).final().at(0) страниц содержит 5 рисунков, 3 таблицы.

В данной лабораторной работе проведено построение упрощённой математической модели системы автоматического регулирования (САР) паровой турбины с помощью одного колебательного звена. Основная цель состояла в поиске параметров этого звена.
...
Цель работы состоит в редуцировании математической модели САР конденсационной паровой турбины без промежуточного перегрева и отбора пара до единичного колебательного звена. Работа проведена с помощью пакетов DifferentialEquations.jl и Plots.jl языка программирования Julia в среде Pluto.
...
Актуальность исследования заключается в упрощении математической модели, что позволит тратить меньше ресурсов на симуляцию.
...
$ T = 1 / omega sqrt(1 + zeta^2) = 1 / #calc.round(ome, digits:3) sqrt(1 + #calc.round(ζ, digits:3)^2) = #calc.round(T, digits:3) $
...
В результате выполнения работы была получена приближенная математическая модель системы автоматического регулирования конденсационной паровой турбины без промежуточного перегрева пара в виде колебательного звена.
