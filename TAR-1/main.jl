...
function find_critical_Ta_corrected(;
	...
	with_theme(theme_latexfonts()) do
		...
		lines!(sol_test, idxs=1)
		resize_to_layout!(fig)

		save("assets/waw.svg", fig)
		fig
	end
end
...
# ╔═╡ f8770d47-ac54-47a4-a4e7-977448e1d3fd
function build_surface(;
	...
	with_theme(theme_latexfonts()) do
    	...
    	ax = Axis3(
			fig[1, 1],
    	    ylabel = L"t, c",
			xlabel = varying_param == :δω ? L"%$(ylabel_latex), %" : L"%$(ylabel_latex)$, c$",
        	zlabel = zlabel_latex,
			protrusions = (50, 10, 0, 0),
			yreversed = true,
			xreversed = true
    	)
		...
		save("assets/surface_plot_$(varying_param).svg", fig)
    	fig
	end
end

# ╔═╡ 8cad7077-90bb-436c-a96b-bdd6f712e1b6
build_surface(
	varying_param=:Ta,
	param_values=range(2.125, 10, length=150)
)

# ╔═╡ dd6c4833-bb28-4258-98a3-81a68fdde9ca
build_surface(
	varying_param=:Ts,
	param_values=range(0.4, 1.0, length=150)
)

# ╔═╡ eb33416d-bac7-4111-96da-318aacfc7b47
build_surface(
	varying_param=:Tπ,
	param_values=range(0.2, 0.6, length=150)
)

# ╔═╡ 2ab9d197-d82b-46e5-adfe-4b41c9e3a9c7
build_surface(
	varying_param=:δω,
	param_values=range(0.08, 0.16, length=150)
)

# ╔═╡ bacf8c9f-80af-41be-8fb2-49ef2353d136
function plot_unified(; varying_param, param_values)
	...
	with_theme(theme_latexfonts()) do
	    ...
	    lines!(ax1, param_values, settling_times, color=:red, label=L"t_п")
    	lines!(ax2, param_values, dynamic_errors, color=:blue, label=L"\varphi_{max}")
    	lines!(ax2, param_values, static_errors, color=:green, label=L"\varphi_\infty")

		axislegend(ax1, position = :lc)
		axislegend(ax2, position = :rc)

	    save("assets/uni_$(varying_param).svg", fig)
    	fig
	end
end

# ╔═╡ 6c014a28-9750-4034-8da3-f13b8a022774
plot_unified(
	varying_param=:Ta,
	param_values=range(2.125, 10, length=150)
)
